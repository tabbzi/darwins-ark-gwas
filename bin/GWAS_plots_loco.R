...
#args = list()
#args[1] = "~/Dropbox (UMass Medical School)/Papers/mutt-paper/data/gwas/"
#args[2] = "MuttPaper"
#args[3] = "DarwinsArk_gp-0.70_snps-only_maf-0.02_geno-0.20_hwe-midp-1e-20_het-0.25-1.00_N-2155"
#args[4] = "Q121A"
#args[5] = "DarwinsArk"
#args[6] = "Q121A"
#args[7] = "1e-06"
...
# args[7] = clumping p-value cutoff
# args[8] = clumping range
# args[9] = clumping r2
# args[10] = outfile
print(args)
fam.file = paste(args[1], "/geno/", args[3], ".fam", sep = "")
print(fam.file)
# assoc.file = paste(args[1], "/assoc/", args[2],"/", args[3], "_", args[4], ".loco.mlma", sep = "")
# clump.file = paste(args[1], "/clump/", args[2],"/", args[3], "_", args[4], "_p-", args[7], "_kb-", args[8], "_r2-", args[9], ".loco.clumped", sep = "")
assoc.file = paste(args[1], "/assoc/", args[2],"/", args[10],".loco.mlma", sep = "")
clump.file = paste(args[1], "/clump/", args[2],"/", args[10], "_p-", args[7], "_kb-", args[8], "_r2-", args[9], ".loco.clumped", sep = "")
genes.file = paste(args[1], "/clump/", args[2],"/", args[10], "_p-", args[7], "_kb-", args[8], "_r2-", args[9], ".loco.clumped.ranges", sep = "")
...
clumped$SP2 = gsub(pattern = "\\(1\\)", replacement = "", x = clumped$SP2)
...
clump$RANGES = as.character(clump$RANGES)
clumpsplit = strsplit(clump$RANGES, split = ",", fixed = T)
clump$RANGES = unlist(lapply(lapply(clumpsplit, FUN = function(x) x[x != "NA"]), function(x) paste(x, collapse = ",")))

# Phenotypes
covar = na.omit(fread(covar.file))
pheno = na.omit(fread(pheno.file))
common = intersect(as.character(covar$V1), intersect(as.character(fam$V1),as.character(pheno$V1)))

if (args[6] != "NA"){
  qcovar = na.omit(fread(qcovar.file))
  common = intersect(intersect(as.character(qcovar$V1),as.character(covar$V1)), intersect(as.character(fam$V1),as.character(pheno$V1)))
}
...
gg.clumps = ggmanClumps(clump,
                        index.snp.column = "SNP",
                        clumps.column = "SP2",
                        group.column = "GROUPS") 

gg.manhattan = ggman(assoc,
                     clumps = gg.clumps,
                     chrom = "Chr",
                     snp = "SNP",
                     bp = "bp",
...
if (min(assoc$p) > 5e-08) {
  gg.manhattan = gg.manhattan[[1]] +
    geom_hline(yintercept = 7.3, colour="#990000", linetype="dashed", alpha = 0.5) +
    labs(x = "Chromosome: Position", y = "-log10(P)") +
    scale_y_continuous(limits = c(0,maxlim),
                       breaks = seq(0,maxlim,1),
                       expand = expansion(mult = c(0,0.1))) +
    guides(fill = F) +
    ggtitle(pheno.string) +
...
ggsave(
  ...
  filename = paste(args[1], "/plots/", args[2], "/", args[10], "_p-", args[7], ".loco.png", sep=""),
  device = "png",
  dpi = 300,
  unit = "in",
  width = 16,
